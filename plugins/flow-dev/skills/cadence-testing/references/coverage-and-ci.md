...
- **Cache the Flow CLI install.** The install script is small but runs on every workflow invocation. A `actions/cache@v4` step keyed on the install script's SHA trims seconds off each run and removes one flaky dependency on upstream network availability.
- **Skip the emulator suite on docs-only PRs.** A `paths-ignore: ['**/*.md', 'docs/**']` filter on the PR workflow keeps the test step from running when a PR touches only Markdown. CI minutes add up; reserving them for changes that could plausibly break tests is worth the ten-second setup.
